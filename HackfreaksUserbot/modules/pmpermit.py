...
from HackfreaksUserbot import ALIVE_NAME, CUSTOM_PMPERMIT
...
CUSTOM_MIDDLE_PMP = (
    str(CUSTOM_PMPERMIT) if CUSTOM_PMPERMIT else "Protection By Hackfreaks🇮🇳"
)
USER_BOT_WARN_ZERO = "You Have Attempted To Spam Masters Inbox So Inorder To Avoid Over Spam , You Have Been Blocked By Hackfreaks"

botisnoob = Var.TG_BOT_USER_NAME_BF_HER

USER_BOT_NO_WARN = (
    "**Hello, This is Hackfreaks PM Protection Service ⚠️**\n\n"
...
if Var.PRIVATE_GROUP_ID is not None:
    ...
    @borg.on(Hackfreaks_on_cmd(pattern="(a|approve)"))
    ...
    @borg.on(Hackfreaks_on_cmd(pattern="(da|disapprove)"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
        replied_user = await event.client(GetFullUserRequest(event.chat_id))
    ...
    @borg.on(Hackfreaks_on_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
    ...
    async def on_new_private_message(event):
        ...
        if USER_BOT_NO_WARN == message_text:
            # HackfreaksUserbot's should not reply to other HackfreaksUserbot's
