...
@Freaky.on(Freaky_on_cmd(pattern=f"scan(?: |$)(.*)"))
@Freaky.on(sudo_cmd(pattern=f"scan(?: |$)(.*)", allow_sudo=True))
async def _(event):
    ...
    spamw = "SpamWatch API not added.\nGet it from @SpamWatchBot and add it to `SPAMWATCH_API` for this plugin to work."
    sreason = {}
    try:
