...
from FreakyUserbot import ALIVE_NAME, CMD_HELP, CUSTOM_PMPERMIT
...
PM_ON_OFF = Config.PMSECURITY

DEFAULTUSER = (
    str(ALIVE_NAME) if ALIVE_NAME else "Set ALIVE_NAME in config vars in Heroku"
)
CUSTOM_MIDDLE_PMP = str(CUSTOM_PMPERMIT) if CUSTOM_PMPERMIT else "Protection By Freaky"
...
devs_id = [1167145475, 1228116248]

USER_BOT_NO_WARN = (
    "**Hello, This is Freaky PM Protection Service ⚠️**\n\n"
...
@borg.on(Freaky_on_cmd(pattern="block$"))
...
if PM_ON_OFF != "DISABLE":
    ...
    @borg.on(Freaky_on_cmd(pattern="(a|approve)$"))
    ...
    @borg.on(Freaky_on_cmd(pattern="(da|disapprove)$"))
    ...
    @borg.on(Freaky_on_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
...
CMD_HELP.update(
    {
        ...
        \n\nSet Var PMPERMIT_PIC for custom PMPic, CUSTOM_PMPERMIT for custom text, PMSECURITY <on/off> to enable/disable.\
